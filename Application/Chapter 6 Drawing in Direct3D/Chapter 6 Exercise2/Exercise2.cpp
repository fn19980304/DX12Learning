...
private:
	...
	std::unique_ptr<MeshGeometryForExiercise6_2> mBoxGeo = nullptr;
...
void BoxApp::Draw(const GameTimer& gt)
{
	...
	// 设置输入装配阶段的顶点位置、顶点颜色、索引缓冲区
	mCommandList->IASetVertexBuffers(0, 1, &mBoxGeo->VPosDataBufferView());
	mCommandList->IASetVertexBuffers(1, 1, &mBoxGeo->VColorDataBufferView());
...
void BoxApp::BuildBoxGeometry()
{
	...
	mBoxGeo = std::make_unique<MeshGeometryForExiercise6_2>();
	mBoxGeo->Name = "boxGeo";

	// 将顶点位置数据复制到CPU内存中
	ThrowIfFailed(D3DCreateBlob(vposbByteSize, &mBoxGeo->VPosDataBufferCPU));
	CopyMemory(mBoxGeo->VPosDataBufferCPU->GetBufferPointer(), verticesPos.data(), vposbByteSize);

	// 将顶点颜色数据复制到CPU内存中
	ThrowIfFailed(D3DCreateBlob(vcolorbByteSize, &mBoxGeo->VColorDataBufferCPU));
	CopyMemory(mBoxGeo->VColorDataBufferCPU->GetBufferPointer(), verticesColor.data(), vcolorbByteSize);

	// 将索引数据复制到CPU内存中
	ThrowIfFailed(D3DCreateBlob(ibByteSize, &mBoxGeo->IndexBufferCPU));
	CopyMemory(mBoxGeo->IndexBufferCPU->GetBufferPointer(), indices.data(), ibByteSize);

	// 通过顶点位置数据创建相应的顶点位置缓冲区资源
	mBoxGeo->VPosDataBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
		mCommandList.Get(), verticesPos.data(), vposbByteSize, mBoxGeo->VPosDataUploader);

	// 通过顶点颜色数据创建相应的顶点颜色缓冲区资源
	mBoxGeo->VColorDataBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
		mCommandList.Get(), verticesColor.data(), vcolorbByteSize, mBoxGeo->VColorDataUploader);

	// 通过索引数据创建相应的索引缓冲区资源
	mBoxGeo->IndexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
		mCommandList.Get(), indices.data(), ibByteSize, mBoxGeo->IndexBufferUploader);

	mBoxGeo->VPosDataByteStride = sizeof(VPosData);
	mBoxGeo->VPosDataBufferByteSize = vposbByteSize;
	mBoxGeo->VColorDataByteStride = sizeof(VColorData);
	mBoxGeo->VColorDataBufferByteSize = vcolorbByteSize;
